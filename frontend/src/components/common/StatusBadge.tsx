...
export const StatusBadge: React.FC<StatusBadgeProps> = React.memo(({ status, isOverdue }) => {
  const getBadgeStyle = () => {
    switch (status) {
      ...
      case "Đã thanh toán":
        return "bg-rose-100 text-rose-800 border-rose-300 dark:bg-rose-950/60 dark:text-rose-200 dark:border-rose-700 font-extrabold";
      default:
  ...
  return (
    <div className="inline-flex items-center gap-1.5 flex-wrap">
      ...
      {isOverdue && status !== "Hoàn thành" && status !== "Đã thanh toán" && (
